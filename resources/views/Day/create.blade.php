...
<body style="background-color:#DEF5E5;">
    ...
    @auth
      ...
      <div class="container2 border border-dark">
        <h2>Introduce el día.</h2>
        <form action="{{route('days.store')}}"
        method="POST">
            @csrf
            <div class="mb-3 mt-3">
              <input type="text" class="form-control" id="name" placeholder="Dia" name="name" value="name">
              <input type="hidden" class="form-control" id="planification_id"  name="planification_id" value={{$planification->id}}>
            </div>
            <button type="submit" class="btn btn-success">Añadir día</button>
          </form>
      </div>
